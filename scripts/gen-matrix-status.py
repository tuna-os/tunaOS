...
def composite_section(criteria, stage, contract, luks, smoke, lifecycle,
    ...
    for variant in sorted(desktops):
        cells = [
            rendered_verdict(variant, d) if d in desktops[variant] else NA
            for d in DESKTOPS
        ]
    ...
    lines = [
        ...
        "",
        (
            "`.github/green-criteria.yml` sets the bar. A cell is green only "
            "when every criterion that blocks it, and applies to it, has a "
            "current affirmative result. A criterion that nobody ran, nobody "
            "asserted, or that CI skipped renders \u2b1c, and it does not "
            "count as satisfied. These criteria block today: "
            + ", ".join(f"`{c}`" for c in blocking)
            + ". These are advisory — the sections below measure them, and "
            "they do not block yet: "
            + ", ".join(f"`{c}`" for c in advisory)
            + ". Unimplemented: "
            + ", ".join(f"`{c}`" for c in unimplemented)
            + ". To graduate a criterion, edit `enforcement:` in that file. "
            "This table and the README count then tighten with no code change."
        ),
        "",
...
def omissions_section(lmatrix, omissions) -> list[str]:
    ...
    return [
        ...
        (
            ...
            "it pulls. That is the same gate new builds pass at build time. So "
            "an image that drops a package in silence, outside "
            "`package-miss-allowlist.txt`, reads ❌ here even when it predates "
            "the gate. A cell whose image the sweep never read — no image, a "
            "pull error, a lost job — is ⬜, not clean."
        ),
        "",
...
def parity_section(lmatrix, parity) -> list[str]:
    ...
    return [
        ...
        "",
        (
            "Green criterion 7 (`parity`), first cadence: `package-parity.yml` "
            "runs `scripts/package-parity.sh --audit` daily, and audits every "
            "desktop package set against its own base. That shape exposes a "
            "build that applies no desktop at all (#858). ❌ covers BROKEN (no "
            "more packages than the base) and suspect (fewer than 25 added). "
            "A diff against each variant upstream reference comes next, and "
...
def build() -> str:
    ...
    out: list[str] = [
        ...
        "by hand. It changes only when a cell moves, so the git history of this "
        "file records when each combination flipped.*",
        "",
    ]
    ...
    out += [
        ...
        "",
        (
            "The denominator is the set `luks-e2e.yml` schedules: every "
            "published desktop image (`build_image`), not only the ones that "
            "ship an ISO. That is wider than the ISO matrix below on purpose. "
            "The browser ISO builder can make an ISO from any image, so the "
            "image-only variants (`sailfin`, `guppy`, `flounder-sid`) need boot "
            "and install coverage too."
        ),
        "",
    ]
    ...
    out += [
        ...
        "",
        (
            "The sweep pulls the **published** image and runs the contract "
            "script against it directly (`podman run`, with no boot). It uses "
            "the same denominator as LUKS E2E above: `build_image`, limited to "
            "the five desktop flavors. This catches a desktop whose packages "
            "never landed, and it does not depend on whether hosted CI can boot "
            "the image at all."
        ),
        "",
    ]
...
EVIDENCE_LINK = re.compile(
    rf"\[([{PASS}{FAIL}{UNTESTED}])\]\(https://github\.com/[^)]+#artifacts\)"
)
...
def structural(block: str, keep_rows: set[tuple[str, str]] | None = None) -> str:
    ...
    for line in block.splitlines():
        ...
        if (
            ...
            continue
        # A green cell links to the run's artifact bundle. Both the glyph and
        # its run URL are live data, so neither belongs in the PR structural
        # drift comparison.
        line = EVIDENCE_LINK.sub(r"\1", line)
...
def main() -> int:
    ...
    if args.check:
        missing_evidence = green_axes_without_evidence(provenance)
        if missing_evidence:
            print(
                "green axes have no evidence URL: " + ", ".join(missing_evidence),
                file=sys.stderr,
            )
            return 1
    ...
    if not args.check:
        ...
        PROV.write_text(json.dumps(
            {"about": "Which run asserted which criterion, when — "
                      ...
                      "MATRIX-STATUS.md; evidence links to the asserting "
                      "run's retained artifact bundle; empty run+date means "
                      "the axis has no current assertion for that cell.",
             "cells": provenance},

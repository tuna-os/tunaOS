...
|---|---|---|---:|---|---|
| `wahoo` (Fedora ELN / EL11 preview) — `base`, `gnome`, `kde`, `cosmic` | #2048; fedora-eln/eln#214 | hanthor | 0 nightly; 0 ISO; **3 LUKS** (gnome/kde/cosmic, monthly) | Base availability measured before the first commit (`eln-bootc` OCI index, 4 arches). Run 34690922548: all four flavors built and were published on amd64/arm64; GNOME/KDE Gate-green; all three desktop-contract jobs green; `TUNAOS_WISHLIST_OK misses=0` throughout. Each contract still records exactly one explicit waiver, `TUNAOS_CODEC_GAP`, because ELN has no functional H.264/H.265 decoder | Admitted as experimental, dispatch-only. Still not Beta: §2 wants boot-gate green **and** a download entry; COSMIC has no boot Gate and there is no download entry. No flavor may be promoted as media-capable while the codec gap stands |

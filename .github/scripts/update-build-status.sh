...

{
	echo "$start"
	echo
	echo "_Generated from the latest completed main-branch build for each variant. A cell is green only when its multi-platform publish gate passed._"
	echo
...
while IFS=$'\t' read -r variant emoji; do
	...
	gates=$(gh api --paginate "repos/${repo}/actions/runs/${run_id}/jobs?per_page=100" \
		--jq '.jobs[] | select(.name | endswith(" / Gate")) | [.name, .conclusion] | @tsv')

	green=0
	failed=()
	for flavor in "${configured[@]}"; do
		gate=$(awk -F '\t' -v suffix="/ ${flavor} / Gate" \
			'index($1, suffix) == length($1) - length(suffix) + 1 { result=$2 } END { print result }' <<<"$gates")
		gate=${gate:-missing}

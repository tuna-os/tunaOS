...
gnome | kde | niri | cosmic | xfce)
	...
	case "$desktop" in
	kde)
		# Re-assert the TunaOS look-and-feel AFTER the desktop packages land.
		#
		# system_files ships kdeglobals with LookAndFeelPackage=org.tunaos.desktop,
		# but it is copied in during the base stage, and Fedora's kde-settings
		# RPM owns the same path — so installing Plasma overwrote our file and
		# bonito:kde shipped LookAndFeelPackage=org.fedoraproject.fedora.desktop
		# (#1008). Setting it here, after the install, is the only ordering that
		# survives.
		#
		# Both locations, deliberately. On Fedora/EL, kde-settings puts its
		# profile dir AHEAD of /etc/xdg in XDG_CONFIG_DIRS, so writing only
		# /etc/xdg would satisfy verify-branding-kde.sh (which greps /etc/xdg
		# first) while Plasma still loaded Fedora's theme — passing the check
		# without fixing the image, which is worse than failing it.
		_kde_set_lnf() {
			local file="$1" want="org.tunaos.desktop"
			mkdir -p "$(dirname "$file")"
			if [[ ! -f "$file" ]]; then
				printf '[KDE]\nLookAndFeelPackage=%s\n' "$want" >"$file"
				return
			fi
			# Replace the key in [KDE] if present, else add it to that section,
			# else append the section — without disturbing anything else in the
			# file (these carry fonts, colour scheme and widget style too).
			awk -v want="$want" '
		...
		_kde_set_lnf /etc/xdg/kdeglobals
		if [[ -d /usr/share/kde-settings/kde-profile/default/xdg ]]; then
			_kde_set_lnf /usr/share/kde-settings/kde-profile/default/xdg/kdeglobals
		fi

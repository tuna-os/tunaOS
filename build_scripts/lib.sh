...
# Platform detection remains source-compatible through this facade, but its
# implementation is isolated so it can be tested without loading package,
# service, and reporting policy.
# shellcheck source=build_scripts/lib/platform.sh
source "$(realpath "$(dirname "${BASH_SOURCE[0]}")")/lib/platform.sh"

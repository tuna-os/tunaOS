...
get_tailscale_version() {
	local version
	version="$(awk -F: '/^tailscale:/{print $2; exit}' /run/context/build_scripts/packages.list 2>/dev/null || true)"
	if [[ -z "${version}" ]]; then
...
install_tailscale_from_url() {
	local tailscale_version rpm_arch repo_base rpm_url attempt

	tailscale_version="$(get_tailscale_version)" || return 1
	rpm_arch="$(uname -m)"

	# Tailscale publishes x86_64, not x86_64_v2
	if [[ "${rpm_arch}" == "x86_64_v2" ]]; then
		rpm_arch="x86_64"
	fi

	if [[ $IS_FEDORA == true ]]; then
		repo_base="https://pkgs.tailscale.com/stable/fedora"
	else
		repo_base="https://pkgs.tailscale.com/stable/centos/${MAJOR_VERSION_NUMBER}"
	fi

	rpm_url="${repo_base}/tailscale-${tailscale_version}.${rpm_arch}.rpm"

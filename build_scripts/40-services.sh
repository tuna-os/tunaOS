...
# This block sits ABOVE the package-manager branching on purpose. Each of the
# apt and pacman/zypper/emerge paths below ends in `exit 0`, so anything placed
# after them runs on dnf/RPM images ONLY. Placed at the end of the file, this
# guard silently never ran on Arch — the very variant the bug was measured on —
# and a full `just build marlin niri` is what caught it: zero mentions of
# profile.d in the build log, and an unguarded umotd.sh in the finished image.
# Being INVOKED by every Containerfile is not the same as being REACHED.

...
- **🔴 Delivery pipeline blocked (08-14, #1657) — NEW since T-8**: the `main` merge-queue rule (tunaos + tunaos-packages) rejects the automerge agent's direct merges (405 "Changes must be made through the merge queue"); **68 merge attempts failed 16:30–16:41Z, 0 tunaos successes, nothing merged since 06:46Z**. ~75+ merge-eligible PRs stranded — incl. every CI fix and strategist planning PRs #1573/#1608/#1639. **Checkpoint decision required**: grant the App a merge-queue bypass actor or switch agent merges to `gh pr merge --queue`; without this, the 08-22 checkpoint cannot distinguish "work done but blocked" from "work not done" — all staff tests below are gated on the same jam.

...
#     echo "${IMAGE_PRETTY_NAME} ${IMAGE_TAG^^} release $VERSION_ID (${VERSION_CODENAME:=Big Bird})" >/etc/system-release
# else
echo "${IMAGE_PRETTY_NAME} release $VERSION_ID Achillobator" >/etc/system-release
sed -i 's/ANACONDA_PRODUCTVERSION=.*/ANACONDA_PRODUCTVERSION=""/' /usr/{,s}bin/liveinst || true
sed -i 's|^Icon=.*|Icon=/usr/share/pixmaps/fedora-logo-icon.png|' /usr/share/applications/liveinst.desktop || true
sed -i "s| Fedora| ${IMAGE_PRETTY_NAME}|" /usr/share/anaconda/gnome/fedora-welcome || true

...
| ~~COPR `jreilly1821/c10s-gnome-50-fresh`, `jreilly1821/c10s-gnome-50`~~ | GNOME 50 desktop stack on EL10 | `manifests/desktops/gnome.yaml` (el10) | **Migrated** (2026-09-03) | Replaced by the tunaos-packages `gnome50-el10-x86_64` build chain: the same 50.0 builds, made in GitHub Actions in a CentOS Stream 10 mock root, delivered as an OCI image of the createrepo_c output (`ghcr.io/tuna-os/tunaos-packages:gnome50-el10-x86_64`) and consumed **by digest** -- pinned in `image-versions.yaml`, bind-mounted at `/run/gnome50-el10-packages`, `unsigned: true` because the digest is the signature. Exactly the utah-packages shape, which is the maintainer directive: no more COPR, build in GitHub. Measured before the switch, the COPR entry had never taken effect anyway -- every EL10 GNOME image shipped the base's 49 (tunaOS#2324). The interim https tier at `repo.tunaos.org/gnome50/10-stream-x86_64` was dropped: that prefix was an rclone mirror of the COPR, signed with the COPR project key, so a gpgcheck=1 consumer could not install from it (run 33750514082). x86_64 only until the chain grows an aarch64 cell |
| COPR `yselkowitz/cosmic-epel` (declared twice) | The whole COSMIC desktop on EL10 — compositor, panel, session, greeter, portal, 20 names in the first block and 3 more under `--nobest --allowerasing` | `manifests/desktops/cosmic.yaml` (el10) | Tier-3, **retires itself at EL10 → EL11** | Was missing from this inventory entirely until 2026-09-13 despite being the sole source of an entire desktop. It needs no Tideforge migration: on Fedora ELN, the EL11 preview base, COSMIC 1.6.0 comes from the distribution's own `eln-extras` — 23 of the manifest's 24 names resolve there, `cosmic-wallpapers` being the only miss, and it is carried as `optional:`. `wahoo:cosmic` builds from that section with **no `copr:` block at all**, green on amd64 and arm64 (run 34690922548). So the answer #2049 asked for is **yes**: if EL11 keeps what ELN has, this COPR ages out at the transition rather than being migrated (#2048, #2049, #1187, #1319) |
| `repo.tunaos.org/fprintd/10-stream-aarch64/` | `fprintd-pam` for aarch64, which CentOS Stream 10 builds only for x86_64 and which `cosmic-greeter` hard-requires | `manifests/desktops/cosmic.yaml` (el10) | Tier-2 (our own repo), **also retires at EL10 → EL11** | Our own build, so not a third-party sourcing problem, but it is a repo that exists only to paper over a base-distro gap. ELN closes the gap at source: `fprintd-1.94.5-6.eln158` resolves from `eln-appstream` on both arches, so the aarch64 shortfall that pinned cosmic off arm64 on EL10 (#732) does not reproduce on this base and the extra repo is not declared in the `eln:` section |
...
2. **Migrate (Q3–Q4)**: move tier-3/✗ sources that have in-house equivalents
   ...
   Migration is not the only exit. Two rows above — `yselkowitz/cosmic-epel`
   and `repo.tunaos.org/fprintd/` — are answered by the base moving on
   instead: both are absent from `cosmic.yaml`'s `eln:` section because
   Fedora ELN already carries what they supply
   ([#2049](https://github.com/tuna-os/tunaos/issues/2049)). Asking whether
   the *next* base ships a package costs a `dnf repoquery` and can retire an
   entry outright, so it is worth doing before opening a Tideforge recipe for
   anything EL10-only.

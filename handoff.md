...
**Status**: Tiers 0–4 fully confirmed passing (9 packages across 5 tiers, including two split-off intra-tier-dependency tiers). `xfdesktop` (tier 4b) is being tested for the first time ever right now — v24 (run 28691620479) is the check-next run.

- ✅ Confirmed green: Tier 0 `xfce4-dev-tools`, Tier 1 `gtk-layer-shell` + `wlr-protocols`, Tier 2 `libxfce4util`+`xfconf`, Tier 3 `libxfce4ui`+`libxfce4windowing`, Tier 3b `garcon`, **Tier 4 `xfce4-panel`+`xfce4-session`+`xfce4-settings`+`thunar`+`tumbler`** (as of v23, run 28691223560 — took v18 through v23, ~6 rounds, to get this tier fully green).
- 🔄 Tier 4b `xfce-desktop-core-xfdesktop` (split out for the same reason garcon was — `xfdesktop` links `thunarx-3` from sibling `thunar`, can't build in the same parallel tier). First-ever run (v23) failed: `%{_datadir}/xfce4/backdrops/` never existed — meson.build installs the actual wallpaper images to `$datadir/backgrounds/xfce` instead, and the `pixmaps/` subdir's icons were never packaged at all. Fixed in v24 (commit 2c82114) — **check this run next**.
- ⏳ Tiers 5–8 (compositor/xfwl4, apps, panel-plugins, meta) — still genuinely unreached. `xfwl4` (tier 5) already has one proactive fix applied (see below) but hasn't actually been build-tested yet.

**Two recurring bug classes, present in every tier so far — budget for both on every new package, don't assume a spec is closer to correct than an actual green CI run has proven:**
1. **meson's `--auto-features=enabled`** (baked into the EL10 `%meson` RPM macro) silently promotes every upstream `type: 'feature', value: 'auto'` meson option to required. One missing pkgconfig lib in the error output can hide several more behind it — check the actual `meson_options.txt` at the exact pinned commit rather than iterating one error at a time.
2. **`%files` written from assumption, not the real install manifest** — specs were drafted before any of these tiers had ever actually built, so paths are frequently wrong (e.g. `tumblerd` assumed at `/usr/bin`, actually under `$libdir/tumbler-1/`; `xfsettingsd` assumed at libexecdir, actually bindir; xfce4-session's shutdown helper assumed at libexecdir, actually `$libdir/xfce4/session/`) or simply incomplete (icons, locale `%find_lang`, GIR/vapi, systemd units, desktop files, polkit actions, man pages, dbus services — all commonly missing on first pass). A few specs also had genuinely stale content: `thunar-bulk-rename` hasn't been a separate binary in the version we're pinned to for a while (merged into `thunar --bulk-rename`).

**Real (non-%files) bugs hit, worth knowing about for other packages**:
- **Intra-tier dependency ordering**: any package whose meson.build references a *sibling* package in the same manifest tier (pkg-config or otherwise) will randomly fail depending on build order. Found twice so far (garcon→libxfce4ui, xfdesktop→thunar) — fixed both times by splitting the dependent package into its own tier that runs after. When adding a new package, check what it actually pkg-config's against, not just what tier the manifest currently puts it in. Sometimes the simpler fix is disabling the feature causing the cross-tier need entirely (did this for `thunar-tpa`, which needs `libxfce4panel-2.0` from sibling `xfce4-panel` — a non-essential integration, so disabled rather than another tier split).
- **Git submodules aren't in GitLab archive tarballs**: `xfce4-settings` needed `protocols/wlr-protocols`. Fixed by packaging `wlr-protocols` as its own tiny package (mirrors Fedora's approach) rather than trying to vendor the submodule tarball — meson.build already prefers pkg-config discovery over the submodule-directory fallback.
- **EL10's base packages can predate upstream conventions**: CentOS Stream 10's `polkit` (v125) doesn't ship the gettext ITS ruleset (`polkit.its`/`.loc`) newer polkit versions provide for translating `.policy` XML files — verified this directly by `rpm -ql polkit` in a real CentOS Stream 10 container rather than trusting Fedora's spec as a stand-in. Fixed by vendoring the tiny static ruleset into `thunar.spec` itself via `GETTEXTDATADIRS`, rather than depending on a base-OS package we don't control.
- **`xsltproc --nonet`** needs the docbook manpages stylesheet resolvable via a local XML catalog rewrite (`docbook-style-xsl` package) — without it, it tries (and fails) to fetch the stylesheet over http.
- **Rust/Cargo packages**: `xfwl4`'s Cargo.lock pulls in `xfconf-sys`/`libxfce4ui-sys`/`libxfce4kbd-private-sys`, needing `xfconf-devel`+`libxfce4ui-devel` via pkg-config, even though nothing in the spec's `%build` looks like it should need them. Found this via `tuna-os/debian-copr`'s parallel Rust build hitting the identical error first — worth cross-checking that repo's build logs before diagnosing any Rust-based github-copr package from scratch.

**GitHub Actions trap, worth remembering for every future tier**: a job whose `needs:` failed gets marked `skipped`, not `failure` — invisible to `select(.conclusion=="failure")` queries. A tier that looks like it "passed" (absent from the failure list) may have never actually run. Always check `.conclusion=="skipped"` too.

**Debugging workflow that's worked well this session**: dispatch → `gh run view <id> --json jobs -q '.jobs[] | select(.conclusion=="failure" or .conclusion=="skipped") | .name'` → pull full job logs (`gh run view <id> --log --job=<databaseId>`, note the databaseId lookup can match multiple jobs if the package name is a substring of another job name — grab the exact `databaseId` not just a name-contains filter) → grep for `error:|ERROR:|ninja: build stopped|Installed \(but unpackaged\)|Directory not found|File not found|FAILED:|cannot locate ITS` → when a dep is missing, check the *actual* upstream `meson.build`/`meson_options.txt` (or `configure.ac`) at the exact pinned commit rather than guessing.

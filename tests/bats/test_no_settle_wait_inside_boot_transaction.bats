...
# kills it.
#
# `checks/e2e-runtime-checks.sh` had already been moved off `--wait`, and
# `verify-base-contract.sh` kept it anyway (tunaOS#2514): a guard that names
# one file does not cover the next one. So this file does not name the
# scripts. It DERIVES them from the units the build actually writes, and a
# new check script is covered before anyone knows it needs to be.

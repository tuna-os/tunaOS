...
The lifecycle ledger from `scripts/gen-matrix-status.py` computes the cell
status and tracks matrix compliance across all 13 declared variants and
142 flavor combinations.
...
5. **Installer & LUKS E2E**: Disk partition setup, LUKS encryption, and first-boot validation (`luks-e2e.yml`).
...
- **No Implicit Skips**: Every cell declared in `.github/build-config.yml` must satisfy the required capabilities or record an active waiver.
- **Waiver Expiry**: Experimental or beta waivers need an assigned owner and an expiration date; expired waivers fail in CI automatically.
- **CI Gate Integration**: `.github/workflows/matrix-status.yml` enforces structure and capability presence in PRs (`--check-structure`). This stops undocumented variants or cell drops.

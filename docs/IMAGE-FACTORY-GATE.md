...
This document establishes the unified completion gate for the image factory and the Definition of Done for all supported TunaOS variants (`.github/build-config.yml`).

## 1. Goal

Deliver an image factory for TunaOS in which the system builds, publishes, and tests every supported variant.
...
- [x] **Package & Desktop Contracts**: Passes contract checks via `verify-desktop-experience.sh` and meets completeness standards (#1294).
- [x] **Declared Outputs**: The build generates and exposes all declared outputs (OCI, ISO, QCOW2, and installers).
...
- [x] **Lifecycle Stream Operations**: Published streams pass the tests for `bootc update`, `bootc rebase`, and `bootc rollback` (`bootc-lifecycle.yml`).
- [x] **Supply-Chain Verification**: Verified keyless signatures via Cosign v3, SPDX SBOMs, and provenance attestations ([VERIFY-ARTIFACTS.md](VERIFY-ARTIFACTS.md)).
- [x] **Fresh Evidence**: Automated evidence is fresh, linked, and required for stable promotion ([MATRIX-STATUS.md](MATRIX-STATUS.md)).
- [x] **Failure Visibility**: Failures in scheduled runs cannot hide behind aggregate status or silent skips.
...
1. **Lifecycle Ledger & Ledger Gate (`#1278`, `#1283`)**:
   - `scripts/gen-matrix-status.py` regenerates `docs/MATRIX-STATUS.md` to track LUKS E2E, Desktop Contract, Bootc Lifecycle, and Installer Smoke.
2. **Installer Coverage (`#1279`)**:
   - `scripts/e2e-installer-gui-checks.sh` tests the frontends of the installer based on declared capabilities.
...
5. **Supply Chain Enforcement (`#1187`, `#1193`)**:
   - Keyless OIDC signatures via Cosign, SPDX SBOM attestations, and ISO verification bundles ([VERIFY-ARTIFACTS.md](VERIFY-ARTIFACTS.md)).
...
8. **Install & Hardware Verification (`#979`, `#1099`, `#989`, `#777`, `#781`)**:
   - Hardware qualification runs for Asahi Apple Silicon (`scripts/verify-asahi-image.sh`) and Intel T2.

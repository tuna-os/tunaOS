...
### wahoo (Fedora ELN, experimental) — 4/4 published on both arches (09-12)
...
**It builds and publishes.** Run 34690922548 built all four flavors on both
architectures, published every manifest, and promoted all four tags. Both
**gnome and kde passed the boot Gate**, and all three desktop-contract jobs
passed. COSMIC's boot Gate remains inapplicable/skipped, so this is not evidence
of a COSMIC VM boot; it is evidence that the package and desktop contracts are
green and the image is published.

| area | state | action |
|---|---|---|
| base amd64/arm64 | built, manifested, cosign-signed, **Promoted** | keep |
| gnome amd64/arm64 | built, desktop-contract job passed, **Gate PASSED**, **Promoted** | keep |
| kde amd64/arm64 | built, desktop-contract job passed, **Gate PASSED**, **Promoted**. Plasma 6.7.4 (plasma-desktop, kwin 6.7.4-2, kscreen); `TUNAOS_BRANDING_KDE_OK` | keep |
| cosmic amd64/arm64 | built, desktop-contract job passed, manifested, signed and **Promoted** on both arches. The earlier AWS `VcpuLimitExceeded` publication blocker is retired; the boot Gate was skipped, not passed | keep published; do not infer VM-boot evidence from the skipped Gate |
| kde display manager | ELN's `kde-desktop` group makes `plasma-login-manager` mandatory and it claims `display-manager.service`; the manifest now overrides the ELN section to `display_manager: plasmalogin` instead of installing an unused SDDM beside it. Both the desktop contract and boot Gate pass with that measured unit | keep the per-section override; Fedora/EL10 continue to use the top-level SDDM default |
...
| desktop contract | All three desktop-contract **jobs pass**. Inside each contract, the codec requirement remains explicitly waived (`missing=1`) and emits `TUNAOS_CODEC_GAP`; this is deliberately not a claim that codecs work | remove the waiver only when a codec source exists |
...
| SBOM attestation | still missing. All four `Attest SBOM` jobs failed against Sigstore in run 34690922548; the classifier treats that upstream outage as non-blocking, so the workflow concluded success and the cosign-signed images promoted | attests itself when Sigstore's write path recovers; do not chase |

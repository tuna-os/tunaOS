...
- **Read an issue**: `gh issue view <number> --comments` to filter comments with `jq` and fetch labels.
...
- **Close an issue**: add an explanation comment with the safe form above, then run `gh issue close <number>`.

### Markdown is never a shell argument

Never put generated or authored Markdown directly in `--body`, `--comment`, or
a shell variable assignment with double quotes. Backticks, `$(...)`, and `$VARIABLE`
are shell syntax: the shell evaluates them before `gh` sees the text and can
splice credentials or command output into a public issue. Escape characters on
backticks alone are not enough.

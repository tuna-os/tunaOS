...
| 24 | `marlin:kde`'s live session starts **intermittently**. Three observed boots of the same image: CI run 34062061739 — `10-ready.png` a 282-byte two-colour PNG, `TUNAOS_LUKS_E2E_INSTALLED_SCREENSHOT rendered=absent`; a local dev ISO — framebuffer **stddev exactly 0** through the full 120s paint wait, `graphical.target` inactive, text `archlinux login:` on ttyS0, SSH refused over TCP *and* vsock; a second local dev ISO — **stddev 0.09, painted on the first attempt, showing the Marlin Installer's welcome page**, with KDE Connect listening on :1716. So the session and `org.tunaos.InstallerKde` are correctly configured and do work; they just do not come up every time. The LUKS gate is green throughout because `--luks` drives fisherman over SSH and never looks at the screen — the hole `live-iso/common/src/customize-live.sh` names: "LUKS E2E can be green on a cell whose GUI install has never once been proven" | Not a configuration defect: the autologin drop-ins are written (`/etc/sddm.conf.d/` + `/etc/plasmalogin.conf.d/`, `[Autologin] User=liveuser Session=plasma`), `sddm.service` is active, and one boot in three reached the installer. An intermittent Plasma-on-Wayland startup under llvmpipe is the shape of it; the failing mount cluster (`tmp.mount`, `var-tmp.mount`, the `sys-kernel-*` API filesystems) appears on the GOOD boot too, so it is not the cause. **Root cause still open**, and it now needs the guest's own journal to go further | Three things make it visible and diagnosable where nothing was before. (1) `scripts/install-checkpoints.py` OCRs the frame and fails the live-desktop checkpoint on the blank boots while passing on the good one — the KDE keywords are MEASURED off the real welcome page. (2) A dev-ISO-only `tunaos-live-debug.service` dumps the DM journal, sshd's journal and listeners, failed units, pending jobs and the autologin drop-ins to the console, which is the serial log the harness already uploads; `iso-e2e.sh` prints that block into the job log whenever the screen checkpoints fail. (3) `iso-e2e.sh` waits for paint before capturing on the install paths, keeps its screenshot when SSH fails, and CI installs tesseract so the keyword assertions actually run |

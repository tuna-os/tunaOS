...
| 42 | `not ok - system has booted (running or degraded)` on **every** installed-system run — measured on wahoo run [34690922548](https://github.com/tuna-os/tunaOS/actions/runs/34690922548) (2026-09-12), the lane's first all-green dispatch: `# system state: starting` at 16.3s on `kde` and 14.6s on `gnome`, both `pass=15 fail=1` with this as the **only** failure, on images that reached a session and passed every other assertion in the file | **Row 33's defect, one property over — the fourth instance in this one file.** `systemctl is-system-running` answers `running` only once the initial boot transaction has drained, and `e2e-runtime-checks.sh` runs from `tunaos-desktop-contract.service`, which is `WantedBy=graphical.target` — so it executes **as a job inside that transaction**. While its `ExecStart` runs the queue is by definition not empty, so the manager answers `starting` on a perfectly healthy boot. Same trap in the obvious fix as row 33: `is-system-running --wait`, or a poll loop, **deadlocks** — it waits on the transaction that is waiting on this unit, until `TimeoutStartSec=90` kills it | Assert the half that does not depend on when we sample: the boot has not landed somewhere **broken**. `maintenance` (emergency/rescue), `stopping`, `offline` and an unreadable state are real regressions whenever you ask; `initializing`/`starting` are healthy-in-progress and `running`/`degraded` healthy-settled. Pinned both directions in `tests/bats/test_graphical_target_assertion.bats`, which now guards all four instances. The stronger claim — *this booted to a graphical desktop* — was never this assertion's to make: `get-default` and the two display-manager checks carry it, time-independently. **Rows 29, 33, 35 and now 42 are the same rule:** an assertion that always says the same thing cannot report a regression |
| 43 | `tunaos-base-contract.service` never produced a verdict on **any** cell — no `TUNAOS_BASE_CONTRACT_OK`, no `_FAIL`. On base cells it sat in its settle wait until `TimeoutStartSec=300` killed it; on desktop cells the harness saw `TUNAOS_DESKTOP_CONTRACT_OK` at ~11s and powered the VM off mid-wait (`bonito:gnome-t2`, run [34768771243](https://github.com/tuna-os/tunaOS/actions/runs/34768771243): `Main process exited, code=killed, status=15/TERM`). Bounding the wait did not help — at a 120s bound the machine settled 14 ms after it expired, and at 240s it settled 12 ms after, with the reported userspace time tracking the **bound** (`2min 7s` then `4min 7s`) rather than the image ([#2501](https://github.com/tuna-os/tunaOS/pull/2501), [#2506](https://github.com/tuna-os/tunaOS/pull/2506), [#2514](https://github.com/tuna-os/tunaOS/issues/2514)) | **Row 42's defect, in the second of the two files that had it — the fifth instance.** `verify-base-contract.sh` used `systemctl is-system-running --wait` from `tunaos-base-contract.service`, which is `WantedBy=multi-user.target`: the wait returns when the manager leaves `starting`, the manager leaves `starting` when the initial transaction drains, and this `Type=oneshot` job is one of the jobs holding it open. Run [34791370259](https://github.com/tuna-os/tunaOS/actions/runs/34791370259) shows it to microsecond resolution — `Startup finished` is logged 10 ms **after** the unit exits, so the bound expiring is what let the machine settle. No bound can be right; a bound only picks how long the deadlock runs. **Why row 42's fix missed it:** that fix pinned the rule in `tests/bats/test_graphical_target_assertion.bats`, whose `--wait` guard names one file — so the guard for this defect was already in the suite, looking elsewhere | Sample once, classify exactly as row 42 does, and let the checks below carry the verdict — `bootc status` is the assertion that matters for a bootc image and it had never once been reachable. `starting` is now a note, not a failure: this unit runs `After=multi-user.target`, so reaching it already proves multi-user was reached, and the state it reports describes the contract rather than the image. `degraded` still passes but now names the failed units, so the verdict is actionable. Guarded in `tests/bats/test_no_settle_wait_inside_boot_transaction.bats`, which **derives** the script list from the units the build writes instead of naming files — the naming is what let instance five hide. **Correction to row 42:** its lesson is right that `--wait` deadlocks and wrong to imply the fix was complete; the rule was applied to one caller, not to the class |

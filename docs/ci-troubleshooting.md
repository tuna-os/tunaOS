...
| 43 | ISO builds finish live-customize, then fail in `podman commit` at exactly 600.0 seconds: skipjack:xfce did so on both rootless and root paths, and marlin:gnome/kde did so with native `overlay` storage | The discriminator is the desktop payload, not one runner or storage driver. Tacklebox's post-customize commit had a literal 600-second deadline, so it turned both a legitimately slow multi-gigabyte layer and a real wedge into the same exit 124. Root and faster storage improve some cells, but neither makes 600 seconds a valid bound for all of them (tunaOS#1893) | Pinned Tacklebox `ae93e9b`, which adds `TBOX_CUSTOMIZE_COMMIT_TIMEOUT`, and made the TunaOS adapter pass 1800 seconds by default while retaining its 4800-second outer deadline. Callers can still choose another non-negative limit or `0`; the outer deadline keeps the complete build bounded. `tests/regressions/test_issue_1893_customize_commit_deadline.py` invokes the real container adapter and proves the setting reaches Tacklebox |

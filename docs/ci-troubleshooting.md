...
| 24 | `marlin:kde` passes LUKS E2E green — `TUNAOS_LUKS_E2E_PASS encrypted=1 passphrase_unlock=1 installed_boot=1` (run 34062061739) — while **its live session never starts a graphical desktop at all**, so nobody could have run the installer from the screen. Reproduced locally on a dev ISO built from the same image: `graphical.target` never activates, the framebuffer measures **stddev=0** (not dim — blank) on every paint poll of the bounded 120s wait, and the console lands on a text `archlinux login:` on ttyS0. The blank frame is NOT a capture artifact: the same run's `00-boot` frame measures **stddev=0.047**, so the virtio-vga device does produce a surface and the screen goes black *after* boot. `TUNAOS_INSTALL_CHECKS_RESULT pass=12 fail=4 desktop=kde`. Six units fail in the live environment: `tmp.mount`, `var-tmp.mount`, `sys-kernel-debug.mount`, `sys-kernel-tracing.mount`, `sys-fs-fuse-connections.mount` (all `Failed with result 'exit-code'`) and `tunaos-offline-store.service` ("Mount tacklebox offline image store"). SSH into the live env is also refused — `kex_exchange_identification: Connection reset by peer` — so the dev ISO's own debug path is unusable on this variant | The gates are serial markers and SSH, and `--luks` drives fisherman over SSH, so the install proceeds whether or not a session ever started — the hole `live-iso/common/src/customize-live.sh` names: "LUKS E2E can be green on a cell whose GUI install has never once been proven". The KDE autologin drop-in IS written (`/etc/sddm.conf.d/live-autologin.conf` + `/etc/plasmalogin.conf.d/`, `[Autologin] User=liveuser Session=plasma`), and `sddm.service` reports active, so this is not the drop-in being missing. The failing mount units are NOT established as the cause and probably are not: that cluster is the signature of API filesystems failing inside an already-mounted namespace, and the yellowfin:niri run in this same harness reports failures at the same early timestamp while reaching a drawn desktop. **Root cause not yet established** — it is specific to the Arch live environment, since the same image installed to disk does reach its greeter | Not fixed. What changed is that it is no longer invisible: `scripts/install-checkpoints.py` fails the live-desktop checkpoint on a blank frame, `iso-e2e.sh` now waits for paint before capturing on the install paths (the capture used to happen too early to mean anything), keeps its evidence screenshot when the SSH check fails, and CI installs `tesseract` so the keyword assertions actually run. Next step is `journalctl -u sddm` inside the guest, which nothing collected so far contains — and getting it needs a shell. Both routes are currently shut: TCP and vsock SSH are refused (sshd accepts, then closes), and the harness's serial chardev is a write-only file, so the text login on ttyS0 cannot be answered. Fixing the live sshd on marlin is therefore a prerequisite for debugging the session, not a side issue |

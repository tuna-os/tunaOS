...
| 24 | `marlin:kde` boots inconsistently: sometimes the live session shows the installer, sometimes the screen is black with **stddev exactly 0** and no `TUNAOS_LIVE_READY` marker, and SSH into the live env is always refused (`kex_exchange_identification: Connection reset by peer`, over TCP *and* vsock). LUKS E2E stays green throughout, because `--luks` drives fisherman over SSH-as-liveuser and never looks at the screen | **Three causes, all named by the guest's own diagnostics dump** (`tunaos-live-debug`, boot with `ready=0`): (1) `flatpak-preinstall.service` was still `start running` at 40s with **`multi-user.target`, `graphical.target` and `tunaos-live-ready.service` all `start waiting`** behind it — one blocker producing both the missing readiness marker and the unstarted session, and the source of the whole intermittency (markers observed at 9s, at 98s, and not at all). (2) `sshd.service` fails permanently: `/usr/share/empty.sshd must be owned by root and not group or world-writable` → `status=255/EXCEPTION` → restart loop, which is exactly what a connection reset at kex looks like from the host. (3) The session itself is fine — SDDM autologin works (`Authentication for user "liveuser" successful`, `Session started true`, `plasma.desktop` selected) — Plasma under llvmpipe is simply slower to first paint than the harness's 120s window on some boots | Not yet fixed; the causes are now specific enough to fix directly. `flatpak-preinstall.service` should not be allowed to order before `multi-user.target` on live media. The sshd failure is a permissions/ownership defect on `/usr/share/empty.sshd` in the Arch image or its squash, and it is what makes every marlin dev ISO unusable for the SSH-driven harness. Getting here needed the dev-ISO diagnostics to be **timer-driven**: the first version was ordered `After=tunaos-live-ready.service`, which is one of the units the blocker was holding, so it stayed silent in exactly this failure |

...
| 43 | EL10 `gnome-nvidia-hwe` reaches an emergency shell: `mount: /sysroot: unknown filesystem type 'xfs'` and `Dependency failed for ostree-prepare-root.service` (albacore run 34745445506; yellowfin run 34738897386) | HWE selects the `coreos-stable-43` akmods bundle and installs its Fedora 43 kernel under EL10's `dracut-107-8.el10`. The initramfs omits XFS without reporting an error. Non-HWE NVIDIA uses the `centos-10` bundle and boots; no EL10 HWE akmods bundle exists. Adding a fixed `--filesystems` list is not safe: EL10 lacks btrfs and that attempt broke otherwise-green NVIDIA cells (#2517). | Keep `gnome-nvidia-hwe` declared for discoverability, but set `build_image: false` on yellowfin, albacore and skipjack so an image that cannot mount its root is not advertised. Re-enable only when an EL10 HWE bundle exists or EL10 dracut can consume the Fedora kernel module tree. Regression coverage: `tests/regressions/test_issue_2518_a_cell_that_cannot_mount_its_root_is_not_built.py`. |

...
| 24 | `marlin:kde` passes LUKS E2E green — `TUNAOS_LUKS_E2E_PASS encrypted=1 passphrase_unlock=1 installed_boot=1` — while its live session never presents the installer. Run 34062061739's `10-ready.png` is a 282-byte two-colour PNG, `TUNAOS_LUKS_E2E_INSTALLED_SCREENSHOT rendered=absent`, and the run's own 238-frame timelapse shows SDDM's greeter: "Select your user and enter password", user LIVEUSER | The gates are serial markers and SSH. `scripts/iso-e2e.sh --luks` drives fisherman over SSH as liveuser, so the install proceeds whether or not anyone could have started it from the screen — the exact hole `live-iso/common/src/customize-live.sh` names: "LUKS E2E can be green on a cell whose GUI install has never once been proven". The live autologin drop-in IS written (CI log: `/etc/sddm.conf.d/live-autologin.conf` and `/etc/plasmalogin.conf.d/live-autologin.conf`, `[Autologin] User=liveuser Session=plasma`), and `desktop-kde.sh`'s own comment predicts this symptom exactly: "Getting this wrong is silent: the greeter simply prompts for a password and the live session (and installer) never starts." Why SDDM on Arch ignores the drop-in is **not yet root-caused** — candidates are the `Session=plasma` basename vs `plasma.desktop`, and Arch's autologin PAM stack | Not fixed yet. What changed is that it is now *visible*: `scripts/install-checkpoints.py` asserts the live frame renders and says what a live session should say, and reported this run's blank frame (advisory). Two evidence defects found alongside it are fixed — the install path photographed `10-ready` before waiting for paint, and the runner had no `tesseract`, so the keyword assertions silently did not run |

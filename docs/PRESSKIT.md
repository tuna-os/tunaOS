...
> Issues that track this work: [#1534](https://github.com/tuna-os/tunaOS/issues/1534) (tech press),
...
> canonical status table for each variant) per the maintainer's press-claim guard.

This is the one-pager for anyone who writes, films, or records about
...
bring **current desktops to Enterprise Linux lifecycles**. It puts GNOME 51
on AlmaLinux/CentOS Stream before the EL point release. It also has KDE,
COSMIC, Niri, XFCE, and Pantheon flavors across eleven base distributions.
...
signed) upgrades, and rollback. Each variant also keeps up with current
desktop releases on enterprise bases. It is a fork of Bluefin (Universal Blue) with a
...
1. **GNOME 51 on EL10 — before the EL point release** (Sept 12, 2026).
   ...
   nautilus, gdm, orca) for AlmaLinux/CentOS Stream. TunaOS builds them with
   its own RPM build chain. Enterprise desktops therefore get GNOME on the
   upstream schedule, and do not wait for the next EL point release.
2. **ARM is next**: Snapdragon X Elite (X13s-class) laptop builds are
   bootc desktops. This is one of the hardware classes for ARM Linux with
   the fastest growth. An **experimental** Apple Silicon (M1/M2) installer
   (bootc-installer-asahi) ships kernel + glue today. The boot payloads are
   still not ready ([#777](https://github.com/tuna-os/tunaos/issues/777)), so
   Apple Silicon is an engineering preview, not a shipped product.
3. **Keyless supply chain**: Sigstore and cosign sign every RPM and DEB,
   with no long-lived keys. TunaOS verifies the signatures against Rekor and
   includes SBOMs. The result is a real chain of trust for desktops.
4. **Pantheon on Ubuntu LTS (Gurnard)**: the elementary-OS desktop as an
   atomic bootc image on Ubuntu 24.04 LTS. It is now **Experimental**,
   per the variant lifecycle.
5. **bootc-migrate**: migrate between Bluefin/Dakota and TunaOS without a
   reinstall — container-native rebase, one transaction.
6. **11 variants, one model**: from Arch and Gentoo to Debian, openSUSE,
   and EL — the same atomic model for updates everywhere.

## Variant matrix (canonical, as of 2026-08-14)

Status terms follow [VARIANT-LIFECYCLE.md](../VARIANT-LIFECYCLE.md). **Stable**
means GA, and **Beta** means tunaos.org/download publishes the variant for
tests. **Experimental** means the variant builds, but is before the
admission gate. This table tracks
...
KDE Plasma, and COSMIC (Rust, from System76, built via Tideforge). The list
also has Niri (scrollable tiling), XFCE (Wayland via xfwl4), and Pantheon
(on Gurnard).

**Architecture**: container images are multi-arch (amd64 + arm64). **ISOs
are amd64-only today** — [#1592](https://github.com/tuna-os/tunaos/issues/1592)
tracks the publication of aarch64 ISOs, which have not shipped yet, so press
copy must not promise aarch64 ISOs.
...
- **Keyless signatures** on the native RPM/DEB repos (R2-hosted), which
  replace the legacy COPR backports
- **Single-maintainer project today** — the project actively looks for its
  first external contributors. There is a pool of 8 good-first-issue tasks
  ahead of Hacktoberfest 2026
...
> updates, keyless-signed verified upgrades, and rollback. Flavors include
> GNOME, KDE Plasma, COSMIC, Niri, XFCE, and Pantheon across AlmaLinux,
...
- Show that GNOME 51 runs on AlmaLinux 10 (the EL10 backport story)
- Show how to verify a keyless signature (cosign verify against Rekor)
- Show the ARM story: Snapdragon X Elite build, plus the preview of the
  experimental installer for Apple Silicon
